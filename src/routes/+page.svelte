<script lang="ts">
	...
	import WordInput from '$lib/com/WordInput.svelte';
	initializeStores();
	...
	function handleMessage(event: CustomEvent<string>) {
		...
		// 使用message填充当前行
		letters[currentRow] = message.split('');
		letters[currentRow].forEach((_, colIndex) => {
			applyGsapAnimation(currentRow, colIndex);
		});
		currentRow++;

		if (
			letters[currentRow - 1].every(
				(letter) => letter === target[letters[currentRow - 1].indexOf(letter)]
			)
		) {
			win = true;
	...
	function rule() {
		...
		const modal: ModalSettings = {
			type: 'alert',
			title: 'Rule',
			body: '绿色表示对的字母对的位置,橙色表示对的字母错的位置,红色表示错误的字母,你有六次机会进行尝试猜出这个五个字母的单词,加油吧(注意中文输入法会可能会导致一次输入两个字母建议切换为英文)',
...
<div class="container">
	...
	{#if !showButton}
		<ul class="text">
			...
			{/each}
			<WordInput on:message={handleMessage} />
...
<footer>
	<p>
		...
		<a href="https://github.com/sevmeowple">
